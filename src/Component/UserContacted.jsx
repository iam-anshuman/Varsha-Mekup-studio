...
export default function UserContacted() {
    ...
    useEffect(()=>{
        async function fetchUsersContacted(index){
            const response = await fetch(`http://localhost:4000/admin/api/getUserContacted?page=${index}`,{
                method:"GET",
                headers:{
        ...
        userContact &&
                    userContact.map((user,index) => {
                        return(
                                ...
                                <div className="mb-2  text-xl font-bold tracking-tight text-gray-900 dark:text-white">Phone : <span className='text-white mx-2 text-base'>{user.phone}</span></div>
                                <div className="mb-2  text-xl font-bold tracking-tight text-gray-900 dark:text-white">Course Selected : <span className='text-white mx-2 text-base'>{user.course}</span></div>
                                <div className='mb-2 text-xl font-bold tracking-normal text-white'>Message :</div>
                                <p className="font-normal text-gray-700 dark:text-gray-400">{user.message}</p>
                                </div>
                )
            })
        }
